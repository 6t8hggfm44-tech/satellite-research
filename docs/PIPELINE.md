...
The installer refuses a conflicting existing definition and accepts an identical one. It enables only this service and registers it if absent. It never kills the current collector or removes its lock. When a manually started collector is still running, macOS's job can encounter the writer lock. The existing collector's health is reported separately and does not establish that the new job can import the module or take over. A registered job with no active PID and a nonzero last exit is reported as **registered startup failed**. Exit code 2 alone is not proof of a lock conflict; inspect the actual error.
